...
const embedder = keypairFromSeed('cc'.repeat(32))
...
describe('fetchVerseData', () => {
  it('returns the change history for the verse from the relays', async () => {
  ...
  it('returns empties for a verse with no merges or citations', async () => {
    const pool = new RelayPool([new MockRelay()])
    await seed(pool)
  ...
  it('counts free citations (cite SDK kind:30710) for the verse with their sources', async () => {
    ...
    const cite = (verse: string, source: string, at: number) =>
      signEvent({ kind: 30710, created_at: at, tags: [['verse', verse], ['translation', TRANSLATION_ID], ['source', source]], content: '' }, embedder.seckey)
